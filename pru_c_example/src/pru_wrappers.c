 ...
 * Inputs:  Kp        -     proportional impedance gain
 * ------------------------------------------------------------------------- */
void readState(state_t s[SIZE_OF_BUFFS], uint8_t bi)
{
...
{
  if(p->flow_control_bit.writeBuffer == 0){
    return 1;
  }
  else{
    return 0;
