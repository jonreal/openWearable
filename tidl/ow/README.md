...
python3 tidl/ow/host/ow_c7x_infer.py tidl/ow/models/ow_tiny_net.bin tidl/ow/models/ow_tiny_io.bin
...
1. **Install** the TI PSDK-RTOS 10.01.00.04 + the **C7000 CGT 4.1.0** (only that compiler is
   needed; see [`tidl/vision-apps/MANIFEST.md`](../tidl/vision-apps/MANIFEST.md)).
2. **Build:**
   ```bash
   make -C tidl/ow/ PSDK=/path/to/ti-processor-sdk-rtos-j721e-evm-10_01_00_04
   #   (or edit the PSDK/CGT defaults at the top of tidl/ow/Makefile, then just: make -C tidl/ow/)
   ```
   Output: `tidl/ow/firmware/vx_app_rtos_linux_c7x_1.out` (stripped). Commit the updated ELF + note it
...
> The optional **fleet** build (`tidl/vision-apps/build-firmware.sh`) is a *separate* path — it stands
...
- **DMA state:** `python3 tidl/ow/host/ow_dru_dump.py [label]` dumps the C7x DRU registers from the A72
