#!/usr/bin/env python3
# openWearable variable-size demonstrator: [1,32,1,1] -> Conv1x1(32->4) + bias -> ReLU -> [1,4,1,1].
# Proves the runtime-sized C7x firmware (OW_MB_INLEN/OUTLEN) on a model that is NOT the 16/8 default:
# the host stages n_in=32 / n_out=4, the firmware sizes its IO copies from those. Same Conv1x1==FC
# (MMA) pattern as gen_tiny_mlp.py, fixed weights (seed 0) -> a known float reference.
import numpy as np
import onnx
from onnx import helper, TensorProto, numpy_helper

np.random.seed(0)
Cin, Cout = 32, 4
W = (np.random.randn(Cout, Cin, 1, 1) * 0.3).astype(np.float32)
b = (np.random.randn(Cout) * 0.1).astype(np.float32)

X = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, Cin, 1, 1])
Y = helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, Cout, 1, 1])
nodes = [
    helper.make_node("Conv", ["input", "W", "b"], ["c"], kernel_shape=[1, 1]),
    helper.make_node("Relu", ["c"], ["output"]),
]
graph = helper.make_graph(nodes, "ow_mlp32x4", [X], [Y],
                          [numpy_helper.from_array(W, "W"), numpy_helper.from_array(b, "b")])
model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 11)])
model.ir_version = 7
from onnx import shape_inference
model = shape_inference.infer_shapes(model)   # give TIDL every tensor's dims
onnx.checker.check_model(model)
onnx.save(model, "/home/root/mlp32x4.onnx")

# fixed input MUST match host/ow_c7x_infer.py: x[j] = j/n_in - 0.5 (ramp in [-0.5, 0.5))
x = ((np.arange(Cin).astype(np.float32) / Cin) - 0.5).reshape(1, Cin, 1, 1)
yref = np.maximum(0.0, (W.reshape(Cout, Cin) @ x.reshape(Cin)) + b)
np.set_printoptions(precision=4, suppress=True)
print("input  x =", x.reshape(-1))
print("FLOAT REF y =", yref)
print("saved /home/root/mlp32x4.onnx")
